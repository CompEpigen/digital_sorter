...
for(c in 1:length(cell_in_interesed)){
  ...
  for( s in 1:length(sample_type)){
    ...
    for(d in 1:length(datasets)){
      print(datasets[d])
      ## read files ####
      table <- readRDS(paste0(outdir,"/",datasets[d],"/",sample_type[s],"_SurfaceGenie_marker_gene_table_subset_arranged.rds"))
  ...
  saveRDS(wenn_list,paste0(outdir, "/", "20220203_public_wenn_list_AT2.rds"))
  
  pdf(paste0(outdir, "/", cell_in_interesed[c], "_markers_upsetplot_barplot20220203.pdf"), width =16, height = 5)
